...
class STDBSCAN(object):
    ...
    def fit_transform(self, df, col_record_id, col_lat, col_lon, col_time,
                      col_cluster='cluster'):
        """
        :param df: DataFrame input
        :param col_record_id: Record ID column name;
        ...
        print("DF: ", df.head())
        # initial setup
        new_df = df[[col_lon, col_lat, col_time]]

        print("DF2: ", new_df.head())
        new_df[col_cluster] = unmarked
        ...
        new_df[col_cluster] = matrix[:, 3]

        new_df[col_record_id] = df[col_record_id]
        return new_df
