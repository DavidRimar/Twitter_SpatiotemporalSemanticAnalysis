...
def main():
    ...
    # GET tweets
    query_df = tweetCrawler.crawl_data_with_session(BristolSet2ST)
    ...
    # DATA PREPROCESSOR
    nlp_pre_processor = NLPPreProcessor(query_df)

    # GET WORD FREQ DATAFRAME PER DAY (for top N words)
    raw_tweet_corpus = nlp_pre_processor.get_raw_tweet_text_array(query_df)
    ...
    # cleanse all tweets
    cleansed_tweet_corpus = nlp_pre_processor.cleanse_all_tweets(
        raw_tweet_corpus)
    ...
    # EXPORT TO CSV
    # df_concat.to_csv('cluster_results/bristol_set2.csv')

    # LOAD DATA
    dataLoader = DataLoader(DATABASE_URI_RDS_TWEETS)

    dataLoader.update_all(df_concat, BristolSet2ST)
